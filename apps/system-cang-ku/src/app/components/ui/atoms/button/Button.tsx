...
export function Button({
  ...
  const baseStyles = 'inline-flex items-center justify-center font-semibold transition-colors duration-200 ease-in-out focus:ring-0 active:border-none';
      ...
      { isLoading ? (
        ...
        <>
          {icon && isValidElement(icon) && (
            <div className={clsx(iconClasses[iconButtonPosition])}>
              {icon}
            </div>
          )}
          {children}
